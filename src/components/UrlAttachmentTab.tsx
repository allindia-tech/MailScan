...
export const UrlAttachmentTab: React.FC<UrlAttachmentTabProps> = ({ analysis }) => {
  const [copiedHash, setCopiedHash] = useState<string | null>(null);

  const { urls, attachments } = analysis;
  ...
  const executableCount = attachments.filter(a => a.flags.isExecutable || a.flags.isScript).length;
      ...
      {attachments.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3 rounded-lg bg-white border border-slate-200 shadow-xs">
            ...
            <div className="text-[10px] text-slate-500 font-mono">Parsed specimens</div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs uppercase tracking-wider font-bold text-slate-900 font-mono">
              Attachment Static & Magic Byte Forensic Triage ({attachments.length})
            </h2>
          </div>
          {criticalMismatchCount > 0 && (
            <span className="px-2.5 py-1 rounded text-[10px] font-mono font-bold bg-rose-600 text-white shadow-xs flex items-center gap-1.5">
              ...
              CRITICAL THREAT: MIME SPOOFING CONFIRMED
            </span>
          )}
        </div>
            ...
            {attachments.map((att) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Hash Locker */}
                  <div className="pt-2 flex flex-wrap items-center justify-between gap-2 text-[11px] font-mono text-slate-600">
                    <div className="flex items-center gap-1.5 truncate">
                      <span>SHA-256:</span>
                      <code className="text-slate-900 font-semibold truncate max-w-sm">{att.sha256 || 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855'}</code>
                    </div>
                    <button
                      onClick={() => handleCopy(att.sha256 || 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855', att.id)}
